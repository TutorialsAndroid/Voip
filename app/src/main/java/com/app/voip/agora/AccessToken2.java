...
public class AccessToken2 {
    ...
    public AccessToken2(String appId, String appCert, int expire) {
        ...
        this.appId = appId;
        this.expire = expire;
        this.issueTs = AgoraUtil.getTimestamp();
        this.salt = AgoraUtil.randomInt();
    }
    ...
    public String build() throws Exception {
        if (!AgoraUtil.isUUID(this.appId) || !AgoraUtil.isUUID(this.appCert)) {
            return "";
        }
        ...
        return getVersion() + AgoraUtil.base64Encode(AgoraUtil.compress(bufferContent.asBytes()));
    }
    ...
    public boolean parse(String token) {
        if (!getVersion().equals(token.substring(0, AgoraUtil.VERSION_LENGTH))) {
            return false;
        }
        byte[] data = AgoraUtil.decompress(AgoraUtil.base64Decode(token.substring(AgoraUtil.VERSION_LENGTH)));
